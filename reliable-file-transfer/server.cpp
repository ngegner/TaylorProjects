...
int process_request(int sockfd, addrinfo *p) {
    ...
    while (true) { // infinite loop to get first request package
        if ((numbytes = recvfrom(sockfd, buf, MAXBUFLEN-1 , 0, (struct sockaddr *)p->ai_addr, &p->ai_addrlen)) == -1) perror("recvfrom");
            ...
            if (!stream) {
                ...
                if ((numbytes = sendto(sockfd, bad_file, strlen(bad_file), 0, p->ai_addr, p->ai_addrlen)) == -1) return -1;
                exit(1);
            }
    ...
    while (file_length != 0) {
        ...
        for (i=1; i<=strlen(packet); i++) stream >> packet[i]; 
        ...
        if (numevents == 0) continue; // timeout
            ...
            if (pollin) {
                if ((numbytes = recvfrom(sockfd, buf, MAXBUFLEN - 1, 0, (struct sockaddr *) p->ai_addr, &p->ai_addrlen)) == -1) continue;
